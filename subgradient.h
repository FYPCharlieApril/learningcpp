...
class Subgradient{
  public:
    Mat<unsigned int> fitPredict(Hypergraph *hg, int train_size, char actfunc, char lossfunc);
    double accuracy;        

  private:
    mat computeDelta(mat f, Hypergraph *hg, int train_size, char actfunc, char lossfunc);
    mat sgm(Hypergraph *hg, int train_size, char actfunc, char lossfunc, int ind, mat& f_a);  
