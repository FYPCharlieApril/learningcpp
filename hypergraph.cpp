...
bool Hypergraph::constructHMat(string filename){
  ...
  tail = hMat; head = hMat; 
  //head = ones<Mat<unsigned int>>(hMat.n_rows, hMat.n_cols);
  //tail = ones<Mat<unsigned int>>(hMat.n_rows, hMat.n_cols);
  weight = ones<Row<unsigned int>>(hMat.n_rows);
...
void Hypergraph::shuffleMat(int time_to_shuffle){
  ...
  //mlpack::data::Save("testtxt/orilMat.txt", lMat);
  //mlpack::data::Save("testtxt/orihMat.txt", hMat);
  //mlpack::data::Save("testtxt/orihead.txt", head);
  //mlpack::data::Save("testtxt/oritail.txt", tail);
  ...
  //printf("canr: %d canc: %d\n", candidate.n_rows, candidate.n_cols);
  mlpack::data::Save("testtxt/candidate.txt", candidate);
  ...
  mlpack::data::Save("testtxt/lMat.txt", lMat);
  mlpack::data::Save("testtxt/hMat.txt", hMat);
  mlpack::data::Save("testtxt/head.txt", head);
  mlpack::data::Save("testtxt/tail.txt", tail);
}
